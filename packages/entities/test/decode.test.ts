...
import { htmlDecodeTree } from "../src/decode";
...
describe("Decode test", () => {
  ...
  it("should not decode legacy entities followed by text in attribute mode", () => {
    expect(decodeHTML("&not", DecodingMode.Attribute)).toBe("¬");

    expect(decodeHTML("&noti", DecodingMode.Attribute)).toBe("&noti");

    expect(decodeHTML("&not=", DecodingMode.Attribute)).toBe("&not=");

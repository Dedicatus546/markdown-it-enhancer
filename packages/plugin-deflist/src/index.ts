...
export const deflist: MarkdownItPlugin = (md) => {
  ...
  // or -1 on fail.
  const skipMarker = (state: StateBlock, line: number) => {
  ...
  const deflist: StateBlockRuleFn = async (
    state,
    startLine,
    endLine,
    silent,
    ...
    OUTER: for (;;) {
      ...
      for (;;) {
        ...
        await state.md.block.tokenize(state, ddLine, endLine, true);

 ...
 * function.
 */
const mapDomain = (domain: string, callback: (str: string) => string) => {
 ...
 */
const ucs2decode = (string: string) => {
  const output = [];
  let counter = 0;
 ...
 */
const ucs2encode = (codePoints: Array<number>) =>
 ...
 */
const digitToBasic = (digit: number, flag: 0 | 1) => {
...
const encode = (input: string) => {
  ...
  const usc2 = ucs2decode(input);

  // Cache the length.
  const inputLength = usc2.length;
  ...
  for (const currentValue of usc2) {
